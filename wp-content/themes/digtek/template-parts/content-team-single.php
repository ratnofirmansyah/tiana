...
$digtek = digtek();
$digtek_meta = get_post_meta(get_the_ID(), 'digtek_team_options', true);
$img_id = get_post_thumbnail_id(get_the_ID()) ? get_post_thumbnail_id(get_the_ID()) : false;
$img_url_val = $img_id ? wp_get_attachment_image_src($img_id, 'full', false) : '';
$img_url = is_array($img_url_val) && !empty($img_url_val) ? $img_url_val[0] : '';
$img_alt = $img_id ? get_post_meta($img_id, '_wp_attachment_image_alt', true) : '';
$designation = isset($digtek_meta['designation']) && !empty($digtek_meta['designation']) ? $digtek_meta['designation'] : '';
$social_icons = isset($digtek_meta['social-icons']) && !empty($digtek_meta['social-icons']) ? $digtek_meta['social-icons'] : '';
$team_skills = isset($digtek_meta['team-skill']) && !empty($digtek_meta['team-skill']) ? $digtek_meta['team-skill'] : '';

?>

<article id="post-<?php the_ID(); ?>" <?php post_class('digtek-details-content-area team-details-section'); ?>>
        ...
        <div class="row g-4">
            <div class="col-lg-5 text-center">
                <div class="details-thumb mb-4 mb-lg-0">
					<?php
                        ...
                        $img_id = get_post_thumbnail_id(get_the_ID()) ? get_post_thumbnail_id(get_the_ID()) : false;
                        $img_url_val = $img_id ? wp_get_attachment_image_src($img_id, 'digtek-team-classic', false) : '';
                        $img_url = is_array($img_url_val) && !empty($img_url_val) ? $img_url_val[0] : '';
                        $img_alt = get_post_meta($img_id, '_wp_attachment_image_alt', true);

                        $team_single_meta_data = get_post_meta(get_the_ID(), 'digtek_team_options', true);
            ...
            <div class="col-lg-7 align-self-center ps-xl-5">
				...
				<div class="details mt-4 pt-3">
                    <h4><?php echo esc_html('About Me', 'digtek'); ?></h4>           
