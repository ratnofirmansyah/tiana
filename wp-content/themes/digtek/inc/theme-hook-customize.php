...
if (!class_exists('Digtek_Customize')) {

    class Digtek_Customize
    {
        /**
         * $instance
         * @since 1.0.0
        ...
        public function __construct()
        {
            ...
            //search popup
            add_action('digtek_after_body', array($this, 'search_popup'));
            //breadcrumb
            add_action('digtek_before_page_content', array($this, 'breadcrumb'));      
            //order comment form
            add_filter('comment_form_fields', array($this, 'comment_fields_reorder'));
            // contact form 7
            add_filter('wpcf7_autop_or_not', '__return_false');
            //mouse move
            add_action('digtek_after_body', array($this, 'mouse_move'));
            // theme_preloader
            add_action('digtek_after_body', array($this, 'theme_preloader'));
             // sidebar
             add_action('digtek_after_body', array($this, 'menu_sidebar'));
             // back_to_top
             add_action('digtek_after_body', array($this, 'back_to_top'));
        ...
        {
            $page_id = digtek()->page_id();
            $check_page = (!is_home() && !is_front_page() && is_singular()) || is_search() || is_author() || is_404() || is_archive() ? true : false;
            $check_home_page = digtek()->is_home_page();
            $page_header_meta = Digtek_Group_Fields_Value::page_container('digtek', 'header_options');
        ...
        <div class="breadcrumb-wrapper bg-cover <?php echo esc_attr($header_variant_class); ?>" style="background-image: url('<?php echo esc_url($background_image_url); ?>');">
            ...
            <div class="left-shape">
                <img src="<?php echo esc_url($breadcrumb_shape_image['url']); ?>" alt="img">
            </div>
            <?php endif; ?>
            <?php
            if (!empty($breadcrumb_shape_image_2['url'])): ?>
            <div class="right-shape">
                <img src="<?php echo esc_url($breadcrumb_shape_image_2['url']); ?>" alt="img">
            </div>
            <?php endif; ?>
            <div class="container">
                <div class="page-heading">
                    <?php
                        ...
                        if (is_archive()) {
                            ...
                            printf('<h1 class="wow fadeInUp" data-wow-delay=".3s">%1$s</h1>', esc_html__('Error 404', 'digtek'));
                        } elseif (is_search()) {
                            printf('<h1 class="wow fadeInUp" data-wow-delay=".3s">%1$s %2$s</h1>', esc_html__('Search Results for:', 'digtek'), get_search_query());
        ...
        {
            if (cs_get_option('mouse_animation')) {
                ?>
                <!--<< Mouse Cursor Start >>-->  
                                ...
                                if( $sidebar_btn_enabled ): ?>
                                    <div class="header-button mt-4">
                                        <div class="main-button">
                                            <a href="<?php echo esc_url($sidebar_btn_text_url); ?>"> <span class="theme-btn"> <?php echo esc_html($sidebar_btn_text); ?> </span></a>
                                        </div>
                                    </div>
                                <?php 
                                endif; ?>
                    ...
                    <div class="search-cell">
                        <form action="<?php echo esc_url(home_url('/')) ?>">
                            ...
                            <input type="text" name="s" class="main-search-input"
                               placeholder="<?php echo esc_attr__('Search....', 'digtek'); ?>">
                            </div>
                        </form>
                    </div>
                </div>
