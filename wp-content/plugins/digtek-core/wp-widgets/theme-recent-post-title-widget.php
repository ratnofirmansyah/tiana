...
class Digtek_Recent_Post_title_Widget extends WP_Widget

{
    public function __construct()
    {
        parent::__construct(
            'digtek_popular_posts_title',
            esc_html__('Digtek: Recent Posts Title', 'digtek-core'),
            array('description' => esc_html__('Display your recent posts, with a thumbnail.', 'digtek-core'))
        );
    }
    ...
    {   
        ...
        if ($recent_articles->have_posts()) : ?>
            ...
            esc_html__(' Oops, there are no posts.', 'digtek-core')
            ?>
        <?php endif; ?>
        </ul>
        <?php
...

    {
        ...
        if (!empty($instance) && $instance['title']) {
            ...
            $title = esc_html__('Recent Posts', 'digtek-core');
        ...
        $post_type = array(
            'post' => esc_html__('Blog Post Type', 'digtek-core'),
            'packages' => esc_html__('Packages Post Type', 'digtek-core'),
            'project' => esc_html__('project Post Type', 'digtek-core'),
            'service' => esc_html__('Service Post Type', 'digtek-core'),
            'courses' => esc_html__('Courses Post Type', 'digtek-core')
        );
        ...
        $orderby_arr = array(
            ...
            'comment_count' => esc_html__('Most Comment', 'digtek-core')
        );

        ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('post_type')) ?>"><?php esc_html_e('Post Type', 'digtek-core'); ?></label>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('no_of_posts')) ?>"><?php esc_html_e('No Of Posts', 'digtek-core'); ?></label>
...
if (!function_exists('Digtek_Recent_Post_title_Widget')) {

    function Digtek_Recent_Post_title_Widget()

    {
        register_widget('Digtek_Recent_Post_title_Widget');
    }

    add_action('widgets_init', 'Digtek_Recent_Post_title_Widget');
