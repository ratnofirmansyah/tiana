 ;(function ($) {
    ...
    $(document).ready(function () {
        /***********************************
         digtek Custom widget js
         ************************************/
        $(document).on('widget-updated',function (e,widget) {
            var widget_id = $(widget).attr('id');
            if (widget_id.indexOf('digtek_about_us_widget') != 1){
                prefetch();
            }
        });

        $('body').off('click','.digtek_flogo_uploader');

        $('body').on('click','.digtek_flogo_uploader',function (e) {
            var el = this;
            ...
            file_uploder_frame.on('insert',function () {
                ...
                var img_prev_container = $(el).siblings('.digtek-logo-preview');
                ...
                $(el).siblings().children('.digtek_logo_id').val(selected_ids.join(","));
                $(el).siblings().children('.digtek_logo_id').trigger('change');
            ...
            file_uploder_frame.on('open',function () {
                ...
                console.log($(el).siblings().children('.digtek_logo_id').val());
                var atts = $(el).siblings().children('.digtek_logo_id').val().split(",");
        ...
        function prefetch () {
            $('.digtek_logo_id').each(function () {
                var attid = $(this).val();
                var container = $(this).parent().siblings('.digtek-logo-preview');
                container.html('');

                if ( attid ){
                    $(this).parent().parent().find('.digtek_flogo_uploader').val('Change Image');
